...
## 5. Раздельный flush: primary vs hist

Primary и hist пишутся в Redis **независимыми пайплайнами** через два отдельных асинхронных task'а:

```
WS message received
       │
       ├─► HSET primary key        (task_flusher: каждые 250мс или 100 команд)
       └─► LPUSH history key       (task_hist_flusher: каждые 300мс, независимый таймер)
```

**Зачем разделили?**
При Binance нагрузке (17k+ msg/s) hist генерирует 20k+ lpush-команд за 250мс.
Смешивание в одном pipeline давало slow flushes 250-800мс.
После разделения primary flush занимает 1-5мс, hist flush — отдельно до 50мс.

### OB hist: ограничение до 10 Hz

OB обновляется со скоростью 10-100 Hz в зависимости от биржи.
Для истории достаточно 10 Hz (одна запись каждые 100мс).

```python
OB_HIST_MIN_INTERVAL_MS = 100  # 10 Hz максимум для ob:hist:*
```

Алгоритм: per-symbol gate через `ob_hist_last_ts[hist_key]`.
Если `ts_ms - last_ts < 100ms` — запись пропускается (счётчик `ob_hist_skipped`).
При смене чанка `ob_hist_last_ts` очищается вместе с `expire_set`.

| Тип | Когда пишется в историю | Записей/час/ключ | Байт/запись |
|-----|------------------------|----------------|------------|
| MD hist | при каждом bookTicker сообщении | ~3,600–12,000 | 25 |
| OB hist | **до 10 Hz** (100мс min интервал) | **~36,000** | 400 |
| FR hist | при каждом funding rate обновлении | ~1-5 | 40 |

**OB hist ограничен до 10 Hz** — snapshot monitor использует forward-fill, более высокая частота не нужна.
...
## 8. Итоговая оценка памяти (OB hist ограничен до 10 Hz)

| Тип | Записей/час/ключ | Байт/запись | Ключей | Итого (4 чанка) |
|-----|----------------|------------|--------|----------------|
| MD hist | ~12,000 | 25 | 8,000 | **~4.8 GB** |
| OB hist | **~36,000** (10 Hz) | 400 | 8,000 | **~230 GB** |
| FR hist | ~2 | 40 | 4,000 | **~1.6 MB** |

**OB hist при 10 Hz:** 10 записей/сек × 3600 сек × 8000 ключей × 400 байт × 4 чанка ≈ 230 GB.
Это выше исходной оценки потому что теперь OB rate-limit работает **per-key** независимо,
а не downsampled глобально. Скорректируйте `maxmemory` Redis при необходимости.
