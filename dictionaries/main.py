...
Назначение
----------
Модуль собирает активные торговые пары с 5 бирж (Binance, Bybit, OKX, Gate.io, Bitget),
...
Выполнение разбито на 5 фаз:
  ...
  Фаза 1 — Параллельный REST-запрос (ThreadPoolExecutor, 5 воркеров):
    Все 5 бирж опрашиваются одновременно. Каждый воркер вызывает
    <exchange>_pairs.fetch_pairs() и сохраняет результаты на диск.
    Итог: сырые списки пар (spot + futures) по каждой бирже.
    При недоступности биржи — предупреждение в stderr, биржа пропускается.

  Фаза 2 — Параллельная WS-валидация (один event loop, asyncio.gather):
    Все 5 бирж валидируются одновременно в одном asyncio event loop.
    ...
    Суммарное время фазы: ~60 с вместо ~300 с при последовательном запуске.
  ...
  Фаза 5 — Symbol IDs и итоговый отчёт.
...
def _fetch_all_exchanges() -> dict:
    """
    Запускает REST-запросы ко всем 5 биржам параллельно.

    Использует ``ThreadPoolExecutor`` (5 воркеров), так как запросы —
    блокирующий I/O (urllib).

    При ошибке отдельной биржи логирует предупреждение в stderr и возвращает
    None для этой биржи — остальные биржи не прерываются.

    Returns
    -------
    dict
        Словарь с результатами по каждой бирже (или None при ошибке)::

            {
                ...
                "bitget": (spot_list, futures_list),     # Bitget
            }
    """
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_key):
            ...
            try:
                ...
                print(f"[WARN] {key} REST failed: {e}", file=sys.stderr)
                results[key] = None
...
async def _validate_all_exchanges(
    ...
    def _safe(res, label: str) -> tuple:
        if isinstance(res, Exception):
            print(f"[WARN] {label} WS validation failed: {res}", file=sys.stderr)
            return ([], [])
        return res

    (abn_spot, abn_fut)     = _safe(_results[0], "bn")
    (abb_spot, abb_fut)     = _safe(_results[1], "bb")
    (aokx_spot, aokx_fut)  = _safe(_results[2], "okx")
    (agate_spot, agate_fut) = _safe(_results[3], "gate")
    (abg_spot, abg_fut)     = _safe(_results[4], "bg")
...
def _generate_symbol_ids() -> dict:
    ...
    Сохраняет результат в config/symbol_ids.json и collectors/symbol_ids.json.
    ...
    out_path = CONFIG_DIR / "symbol_ids.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(sym_ids, indent=2, sort_keys=True), encoding="utf-8")
    return sym_ids


def _generate_source_ids() -> dict:
    """
    Присваивает целочисленный ID каждому рынку (биржа + тип).
    Порядок фиксированный и алфавитный по ключу.
    Сохраняет результат в config/source_ids.json.
    Возвращает словарь {source: id}.
    """
    sources = sorted(SUBSCRIBE_MAP.keys())
    source_ids = {src: idx for idx, src in enumerate(sources)}
    out_path = CONFIG_DIR / "source_ids.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(source_ids, indent=2, sort_keys=True), encoding="utf-8")
    return source_ids
...
def main() -> None:
    ...
    Порядок выполнения
    ------------------
    1. **Фаза 1 — Параллельный REST** (все 5 бирж одновременно):
       Получение сырых списков пар через публичные REST API.
       Результаты сохраняются в ``<exchange>/data/``.

    2. **Фаза 2 — Параллельная WS-валидация** (все 5 бирж одновременно):
    ...
    print("[Фаза 1/5] REST API: получение пар со всех бирж параллельно...", flush=True)
    t_rest = time.time()

    rest_results = _fetch_all_exchanges()

    bn_spot,       bn_fut       = rest_results["bn"]     or ([], [])
    bb_spot,       bb_fut       = rest_results["bb"]     or ([], [])
    okx_spot_norm, okx_fut_norm = rest_results["okx"]   or ([], [])
    gate_spot_norm,gate_fut_norm= rest_results["gate"]  or ([], [])
    bg_spot,       bg_fut       = rest_results["bitget"] or ([], [])
    ...
    # ── Фаза 2: Параллельная WS-валидация ────────────────────────────────────
    print(f"\n[Фаза 2/5] WS-валидация: все биржи параллельно ({DURATION_SECONDS} сек)...", flush=True)
    t_ws = time.time()
    ...
    # ── Фаза 3: Пересечения ───────────────────────────────────────────────────
    print("\n[Фаза 3/5] Создание пересечений (combination/)...", flush=True)
    active = {
    ...
    print("\n[Фаза 4/5] Создание файлов подписки (subscribe/)...", flush=True)
    sub_counts = _make_subscribe_files()
    for key, cnt in sub_counts.items():
        print(f"        {cnt:4d} пар  {key}")

    # ── Фаза 5: Symbol IDs, Source IDs и отчёт ───────────────────────────────
    print("\n[Фаза 5/5] Symbol IDs, Source IDs и итоговый отчёт...", flush=True)
    sym_ids = _generate_symbol_ids()
    print(f"       {len(sym_ids)} уникальных символов → config/symbol_ids.json")
    source_ids = _generate_source_ids()
    print(f"       {len(source_ids)} источников        → config/source_ids.json")
